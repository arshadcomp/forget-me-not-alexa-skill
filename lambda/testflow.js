...
var context = {
    'succeed': function (data) {
        ...
        if (data.response.outputSpeech!==undefined) {
        ...
        // For repromptText
        if (data.response.reprompt!==undefined) {


        var promptToSay = data.response.reprompt.outputSpeech.ssml;
        promptToSay = promptToSay.replace('<speak>', '    ');
        promptToSay = promptToSay.replace('</speak>', '');

        if (data.response.reprompt.outputSpeech) {
            console.log = OriginalConsoleLog;
            console.log('\x1b[36m%s\x1b[0m ', promptToSay);
        }
      }
    ...
    var eventJSON =
        {
            "session": {
                ...
                "attributes": sa,
                "user": {
                    "userId": "amzn1.ask.account.VO3PVTGF563MOPBY.etc.etc1"
                },
                "new": newSession
            },
            request,
